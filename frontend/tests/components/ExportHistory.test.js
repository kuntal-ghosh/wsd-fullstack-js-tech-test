...
describe('ExportHistory', () => {
  ...
  describe('Export History Table Display', () => {
    ...
    it('should display each export history entry correctly', () => {
      ...
      expect(firstRow.find('[data-test="export-date"]').text()).toContain('2024-05-01')
      expect(firstRow.find('[data-test="export-format"]').text()).toContain('CSV')
      expect(firstRow.find('[data-test="export-status"]').text()).toContain('Completed')
      expect(firstRow.find('[data-test="export-records"]').text()).toContain('150')
  ...
  describe('Pagination Functionality', () => {
    ...
    it('should load next page when pagination is clicked', async () => {
      ...
      expect(mockFetchExports).toHaveBeenCalledWith(expect.objectContaining({
        page: 2
      }))
    })
  ...
  describe('Export Metadata Display', () => {
    it('should show filter summary when expanding an export row', async () => {
      ...
      const filterDetails = wrapper.find('[data-test="filter-details-export-1"]')
      expect(filterDetails.exists()).toBe(true)
      expect(filterDetails.text()).toContain('Status: pending, in-progress')
      expect(filterDetails.text()).toContain('Date Range: 2024-01-01 to 2024-04-30')
    })
    ...
    it('should format different types of filters correctly', async () => {
      ...
      const filterDetails = wrapper.find('[data-test="filter-details-export-2"]')
      expect(filterDetails.exists()).toBe(true)
    ...
    it('should handle missing filter data gracefully', async () => {
      ...
      const expandButton = wrapper.find('[data-test="expand-row-export-missing"]')
      await expandButton.trigger('click')
      
      // Check if filter details are displayed gracefully
      const filterDetails = wrapper.find('[data-test="filter-details-export-missing"]')
      expect(filterDetails.exists()).toBe(true)
  ...
  describe('Export Actions (Download, Delete, Retry)', () => {
    ...
    it('should trigger download when button is clicked', async () => {
      ...
      const downloadButton = wrapper.find('[data-test="download-button-export-1"]')
    ...
    it('should show download progress during download', async () => {
      ...
      const downloadProgress = wrapper.find('[data-test="download-progress-export-1"]')
    ...
    it('should show confirmation dialog before deleting', async () => {
      ...
      const confirmDialog = wrapper.find('[data-test="delete-confirmation-dialog"]')
      expect(confirmDialog.exists()).toBe(true)
      expect(confirmDialog.text()).toContain('Are you sure you want to delete this export?')
    })
    ...
    it('should disable action buttons when loading', async () => {
      ...
      const buttons = wrapper.findAll('[data-test^="download-button"], [data-test^="delete-button"], [data-test^="retry-button"]')
  ...
  describe('Empty and Error States', () => {
    ...
    it('should provide a refresh button when error occurs', async () => {
      const errorMessage = 'Failed to load export history'
      const mockFetchExports = vi.fn().mockRejectedValue(new Error(errorMessage))
    ...
    it('should handle API error responses for actions gracefully', async () => {
      ...
      exportStore.downloadExport.mockRejectedValueOnce(new Error('Download failed'))
  ...
  describe('Filters and Sorting', () => {
    ...
    it('should apply format filter when selected', async () => {
      ...
      expect(exportStore.fetchExports).toHaveBeenCalledWith(expect.objectContaining({
        format: 'csv'
      }))
    })
    ...
    it('should apply status filter when selected', async () => {
      ...
      const completedOption = wrapper.find('[data-test="status-option-completed"]')
    ...
    it('should allow date range filtering', async () => {
      ...
      // Apply date filters
      const applyFiltersButton = wrapper.find('[data-test="apply-filters-button"]')
    ...
    it('should clear all filters when reset button clicked', async () => {
      ...
      // Now clear filters
      const resetFiltersButton = wrapper.find('[data-test="reset-filters-button"]')
