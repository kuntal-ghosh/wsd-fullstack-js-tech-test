...
const ExportProgress = {
  name: 'ExportProgress',
  template: '<div class="export-progress"><div data-test="export-format">CSV</div><div data-test="export-filename">task-export.csv</div><div data-test="progress-text">45%</div><div data-test="progress-bar" aria-valuenow="45" class="bg-primary"></div><div data-test="status-text">Processing</div><div data-test="export-metadata">5KB, 100 records</div></div>',
  props: ['exportId', 'showActions', 'exportData'],
  emits: ['error', 'cancel', 'retry'],
  setup(props, { emit }) {
    return {
...
describe('ExportProgress', () => {
  ...
  describe('Export Status and Progress Display', () => {
    it('should display export format and filename', () => {
      ...
      expect(wrapper.find('[data-test="export-format"]').text()).toContain(
        'CSV'
      )
      expect(wrapper.find('[data-test="export-filename"]').text()).toContain(
        'task-export.csv'
      )
    })
    ...
    it('should display progress bar with the correct status color', async () => {
      ...
      expect(wrapper.find('[data-test="progress-bar"]').classes()).toContain(
        'bg-primary'
      )
      ...
      wrapper = createWrapper(
        {},
        { ...mockExportData, status: 'completed', progress: 100 }
      )
      expect(wrapper.find('[data-test="progress-bar"]').classes()).toContain(
        'bg-success'
      )
      ...
      wrapper = createWrapper(
        {},
        { ...mockExportData, status: 'failed', error: 'Export failed' }
      )
      expect(wrapper.find('[data-test="progress-bar"]').classes()).toContain(
        'bg-error'
      )
    ...
    it('should show appropriate status text', () => {
      // Test processing status
      wrapper = createWrapper({}, { ...mockExportData, status: 'processing' })
      expect(wrapper.find('[data-test="status-text"]').text()).toContain(
        'Processing'
      )
  ...
  describe('Real-time Progress Updates via Socket.IO', () => {
    it('should update progress bar when export progress changes', async () => {
      ...
      expect(
        wrapper.find('[data-test="progress-bar"]').attributes('aria-valuenow')
      ).toBe('45')
      expect(wrapper.find('[data-test="progress-text"]').text()).toContain(
        '45%'
      )
  ...
  describe('Export Status Indicators with Colors', () => {
    it('should apply appropriate colors to status indicators', () => {
      // Processing status
      wrapper = createWrapper({}, { ...mockExportData, status: 'processing' })
      expect(wrapper.find('[data-test="status-chip"]').classes()).toContain(
        'bg-primary'
      )
      expect(wrapper.find('[data-test="status-icon"]').text()).toContain(
        'mdi-sync'
      )
    ...
    it('should animate the progress icon for processing exports', () => {
      wrapper = createWrapper({}, { ...mockExportData, status: 'processing' })
      expect(wrapper.find('[data-test="status-icon"]').classes()).toContain(
        'rotating'
      )
    })
  ...
  describe('Export Failure States with Error Messages', () => {
    ...
    it('should handle different error scenarios with appropriate messages', () => {
      ...
      expect(wrapper.find('[data-test="error-message"]').text()).toContain(
        'Export timed out after 30 seconds'
      )
  ...
  describe('Action Buttons', () => {
    ...
    it('should not show action buttons when showActions is false', () => {
      wrapper = createWrapper(
        { showActions: false },
        { ...mockExportData, status: 'processing' }
      )
      ...
      wrapper = createWrapper(
        { showActions: false },
        { ...mockExportData, status: 'completed' }
      )
      expect(wrapper.find('[data-test="download-button"]').exists()).toBe(false)
    })
