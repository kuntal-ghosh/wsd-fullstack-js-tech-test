...
describe('Export Workflow End-to-End Tests', () => {
  ...
  describe('Complete Export Workflow', () => {
    it('should complete full export workflow from UI to download', async () => {
      ...
      const progressComponent = wrapper.findComponent({ name: 'ExportProgress' });
      if (progressComponent.exists()) {
        const downloadButton = progressComponent.find('[data-test="download-button"]');
        expect(downloadButton.exists()).toBe(true);
        
        // 9. Test download functionality
        await downloadButton.trigger('click');
        expect(apiClient.downloadExport).toHaveBeenCalledWith('test-export-123', expect.any(String));
      }
      ...
      if (exportHistory.exists()) {
        ...
        const historyDownloadButton = exportHistory.find('[data-test="download-button-test-export-123"]');
  ...
  describe('Real-time Socket.IO Integration', () => {
    it('should properly handle all real-time export events', async () => {
      ...
      if (socketHandlers['export-progress']) {
        ...
        const updatedExport = exportStore.exports.find(e => e._id === 'socket-test-export');
      ...
      if (socketHandlers['export-completed']) {
        ...
        expect(updatedExport.downloadUrl).toBe('/api/exports/socket-test-export/download');
      }
      ...
      if (socketHandlers['export-list-update']) {
        ...
        const newExport = exportStore.exports.find(e => e._id === 'new-export-via-socket');
      ...
      if (socketHandlers['export-failed']) {
        ...
        const failedExport = exportStore.exports.find(e => e._id === 'new-export-via-socket');
  ...
  describe('Error Handling in Export Flow', () => {
    it('should handle errors during export initiation', async () => {
      // Mock API error
      apiClient.createExport.mockRejectedValue(new Error('Server temporarily unavailable'));
    ...
    it('should handle export processing failures', async () => {
      ...
      if (socketHandlers['export-failed']) {
        ...
        // Verify export was marked as failed
        const failedExport = exportStore.exports.find(e => e._id === 'failing-export');
        expect(failedExport.status).toBe('failed');
