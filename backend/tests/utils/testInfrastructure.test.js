...
import { 
  ensureTestExportDir, 
  cleanupTestExportDir, 
  createTestExportFile, 
  testExportFileExists,
  getTestExportDir 
} from './fileHelpers.js';
import { loadTestConfig, getTestExportConfig, isTestEnvironment } from './testConfig.js';
...
describe('Test Infrastructure', { timeout: 10000 }, () => {
  ...
  describe('Test Configuration', () => {
    ...
    test('should provide export configuration', () => {
      ...
      assert(typeof config.exportDir === 'string', 'Export dir should be a string');
      assert(typeof config.fileTTL === 'number', 'File TTL should be a number');
      assert(typeof config.maxFileSize === 'number', 'Max file size should be a number');
    });
  });
  ...
  describe('File System Helpers', () => {
    test('should create test export directory', async () => {
      ...
      assert(dir.includes('test-exports'), 'Should contain test-exports in path');
    });
  ...
  describe('Mock Data Generation', () => {
    ...
    test('should generate multiple mock tasks', () => {
      ...
      tasks.forEach((task, index) => {
        assert(task.title.includes(`${index + 1}`), 'Should have unique titles');
      });
    });
    ...
    test('should generate mock export data', () => {
      ...
      assert(['csv', 'json'].includes(exportData.format), 'Should have valid format');
      assert(['processing', 'completed', 'failed'].includes(exportData.status), 'Should have valid status');
      assert(typeof exportData.filters === 'object', 'Should have filters object');
    });
